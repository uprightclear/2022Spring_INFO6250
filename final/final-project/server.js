...
app.get('/api/surveys', (req, res) => {
    ...
    res.json(users.getUserData(username).getSurveys());
});
...
app.post('/api/surveys', (req, res) => {
    ...
    const todoList = users.getUserData(username);
    const id = todoList.addSurvey(course);
    res.json(todoList.getSurvey(id));
});
...
app.patch('/api/ranking/:id', (req, res) => {
    const sid = req.cookies.sid;
    const username = sid ? sessions.getSessionUser(sid) : '';
    if(!sid || !username) {
        res.status(401).json({ error: 'auth-missing' });
    ...
    const todoList = users.getUserData(username);
    if(!todoList.surveyContains(id)) {
        res.status(404).json({ error: `noSuchId`, message: `No Survey with id ${id}` });
        return;
    }
    todoList.updateSurvey(id, {task, score});
    res.json(todoList.getSurvey(id));
});
...
app.delete('/api/surveys/:id', (req, res) => {
    ...
    const { id } = req.params;
    const todoList = users.getUserData(username);
    const exists = todoList.surveyContains(id);
    if(exists) {
        todoList.deleteSurvey(id);
    }
    res.json({ message: exists ? `survey ${id} deleted` : `survey ${id} did not exist` });
});
